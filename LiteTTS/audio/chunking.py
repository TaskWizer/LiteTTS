...
class TextChunker:
    ...
    def chunk_text(self, text: str) -> list[TextChunk]:
        ...
        logger.info(
            f"Text chunked into {len(chunks)} chunks using {self.config.strategy.value} strategy"
        )
        return chunks
    ...
    def _chunk_by_sentences(self, text: str) -> list[TextChunk]:
        ...
        for i, sentence in enumerate(sentences):
            ...
            if (
                ...
                and current_chunk
                and len(current_chunk) >= self.config.min_chunk_size
            ):
    ...
    def _chunk_by_phrases(self, text: str) -> list[TextChunk]:
        ...
        for i, phrase in enumerate(phrases):
            ...
            if (
                len(current_chunk) + len(phrase) > self.config.max_chunk_size
                and current_chunk
                and len(current_chunk) >= self.config.min_chunk_size
            ):
                chunks.append(
