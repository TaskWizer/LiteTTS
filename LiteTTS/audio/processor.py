...
class AudioProcessor:
    ...
    def create_audio_segment(
        self, audio_data: np.ndarray, sample_rate: int, format: str = "wav"
    ) -> AudioSegment:
        """Create an AudioSegment from raw audio data"""
        return AudioSegment(audio_data=audio_data, sample_rate=sample_rate, format=format)
    ...
    def apply_crossfade(
        self, segments: list[AudioSegment], fade_duration: float = 0.1
    ) -> AudioSegment:
        ...
        for segment in segments[1:]:
            ...
            if len(fade_out_result.audio_data) > overlap_samples:
                ...
                result = AudioSegment(
                    audio_data=combined_audio, sample_rate=result.sample_rate, format=result.format
                )
            else:
    ...
    def convert_format(self, audio_segment: AudioSegment, target_format: str, **kwargs) -> bytes:
        """Convert audio segment to target format"""
        return self.format_converter.convert_format(
            audio_segment.audio_data, audio_segment.sample_rate, target_format, **kwargs
        )

    def validate_audio(self, audio_segment: AudioSegment) -> dict[str, Any]:
        """Validate audio segment and return validation results"""
        validation_result = {"is_valid": False, "errors": [], "warnings": [], "info": {}}
        ...
        try:
            ...
            if audio_segment.sample_rate <= 0:
                validation_result["errors"].append(
                    f"Invalid sample rate: {audio_segment.sample_rate}"
                )
                return validation_result
            ...
            if max_amplitude > 1.0:
                validation_result["warnings"].append(
                    f"Audio amplitude exceeds 1.0: {max_amplitude}"
                )
