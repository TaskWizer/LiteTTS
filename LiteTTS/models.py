...
class TTSRequest(BaseModel):
    ...
    time_stretching_enabled: bool | None = Field(
        default=None, description="Enable time-stretching optimization (beta)"
    )
    time_stretching_rate: int | None = Field(
        default=None, ge=10, le=100, description="Time-stretching compression rate (10-100%)"
    )
    time_stretching_quality: str | None = Field(
        default=None, description="Time-stretching quality: low, medium, high"
    )
...
def parse_voice_attributes(voice_name: str) -> dict[str, str]:
    ...
    region_info = region_mapping.get(
        region_char, {"region": "unknown", "language": "en-us", "flag": "🌍"}
    )
...
def create_voice_metadata(voice_name: str) -> VoiceMetadata:
    ...
    return VoiceMetadata(
        ...
        description=descriptions.get(
            voice_name, f"{attributes['region'].title()} {attributes['gender']} voice"
        ),
    )
