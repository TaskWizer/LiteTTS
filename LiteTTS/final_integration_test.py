...
def test_whisper_optimization():
    ...
    try:
        ...
        configs = [("distil-small.en", "int8"), ("base", "int8"), ("tiny", "int8")]
        ...
        for model_name, compute_type in configs:
            try:
                ...
                logger.info(
                    f"  Success: {result.success}, RTF: {result.rtf:.3f}, Memory: {result.memory_usage_mb:.1f}MB"
                )
    ...
    try:
        ...
        for duration in test_durations:
            if duration <= cloner.max_audio_duration:
                ...
                logger.info(
                    f"  Success: {result.success}, Quality: {result.quality_score:.3f}, Rate: {processing_rate:.1f}x"
                )
...
def test_configuration_system():
    ...
    try:
        ...
        logger.info(
            f"  Override working: {'✅ YES' if new_settings.default_model == 'base' else '❌ NO'}"
        )
