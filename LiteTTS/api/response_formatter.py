...
class ResponseFormatter:
    ...
    async def format_audio_response(
        self,
        ...
        try:
            ...
            headers = self._create_audio_headers(audio_segment, format, processing_time, cache_hit)

            if stream:
                # Return streaming response
                return StreamingResponse(
                    self._create_audio_stream(audio_bytes), media_type=content_type, headers=headers
                )
            else:
    ...
    async def format_streaming_audio_response(
        self, audio_segment: AudioSegment, format: str, processing_time: float
    ) -> StreamingResponse:
    ...
    def _create_streaming_headers(
        ...
        headers.update(
            {"Transfer-Encoding": "chunked", "Connection": "keep-alive", "X-Streaming": "true"}
        )
    ...
    def format_health_response(
        self, is_healthy: bool, system_info: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def format_voice_list_response(self, voices: dict[str, Any], emotions: list) -> dict[str, Any]:
    ...
    def format_stats_response(self, stats: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"statistics": stats, "timestamp": self._get_timestamp(), "version": "1.0.0"}

    def create_sse_response(self, audio_segment: AudioSegment, format: str) -> StreamingResponse:
