...
class PerformanceTest:
    ...
    def test_single_synthesis(self, text: str, voice: str = "af_heart") -> dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={"input": text, "voice": voice, "response_format": "mp3", "speed": 1.0},
                timeout=30,
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✅ Synthesis successful: {len(text)} chars, {synthesis_time:.3f}s, RTF: {rtf:.3f}"
                )
                return result
            else:
    ...
    def run_performance_test(
        self, test_cases: list[str], voice: str = "af_heart", iterations: int = 3
    ) -> dict[str, Any]:
        """Run comprehensive performance test"""
        print(
            f"🧪 Running performance test with {len(test_cases)} test cases, {iterations} iterations each"
        )
        ...
        for i, text in enumerate(test_cases, 1):
            ...
            if case_results and all(r["success"] for r in case_results):
                ...
                print(
                    f"    RTF: avg={statistics.mean(rtfs):.3f}, min={min(rtfs):.3f}, max={max(rtfs):.3f}"
                )
                print(
                    f"    Time: avg={statistics.mean(times):.3f}s, min={min(times):.3f}s, max={max(times):.3f}s"
                )
    ...
    def print_summary(self, summary: dict[str, Any]):
        ...
        print(
            f"📊 Tests: {summary['successful_tests']}/{summary['total_tests']} successful ({summary['success_rate']:.1%})"
        )
...
def main():
    ...
    parser.add_argument(
        "--iterations", type=int, default=3, help="Number of iterations per test case"
    )
