...
class ComprehensiveTestSuite:
    ...
    def test_basic_api(self) -> dict[str, Any]:
                ...
                results["tests"].append(
                    {
                        "name": "basic_tts",
                        "status": "failed",
                        "error": f"HTTP {response.status_code} or small audio",
                    }
                )
        except Exception as e:
            results["failed"] += 1
        ...
        try:
            ...
            results["failed"] += 1
            results["tests"].append(
                {"name": "voice_validation", "status": "failed", "error": str(e)}
            )
    ...
    def test_ssml_background(self) -> dict[str, Any]:
        ...
        for bg_type in background_types:
            ...
            try:
                ...
                if (
                    response.status_code == 200 and len(response.content) > 5000
                ):  # Background audio should be larger
                    results["passed"] += 1
                    results["tests"].append(
                        {"name": f"ssml_background_{bg_type}", "status": "passed"}
                    )
                else:
                    results["failed"] += 1
                    results["tests"].append(
                        {
                            "name": f"ssml_background_{bg_type}",
                            "status": "failed",
                            "error": f"HTTP {response.status_code} or small audio",
                        }
                    )
                ...
                results["failed"] += 1
                results["tests"].append(
                    {"name": f"ssml_background_{bg_type}", "status": "failed", "error": str(e)}
                )
        ...
        try:
            ...
            results["failed"] += 1
            results["tests"].append(
                {"name": "ssml_volume_control", "status": "failed", "error": str(e)}
            )
    ...
    def test_voice_showcase(self) -> dict[str, Any]:
        ...
        if len(audio_samples) >= 50:  # Should have most voice samples
            results["passed"] += 1
            results["tests"].append(
                {"name": "audio_samples_exist", "status": "passed", "count": len(audio_samples)}
            )
        else:
            results["failed"] += 1
            results["tests"].append(
                {
                    "name": "audio_samples_exist",
                    "status": "failed",
                    "error": f"Only {len(audio_samples)} samples found",
                }
            )
        ...
        if showcase_path.exists():
            try:
                ...
                results["failed"] += 1
                results["tests"].append(
                    {"name": "showcase_content_valid", "status": "failed", "error": str(e)}
                )
        else:
            results["failed"] += 1
            results["tests"].append(
    ...
    def test_dashboard(self) -> dict[str, Any]:
        ...
        try:
            ...
            results["failed"] += 1
            results["tests"].append(
                {"name": "dashboard_accessible", "status": "failed", "error": str(e)}
            )
        ...
        try:
            ...
            results["failed"] += 1
            results["tests"].append(
                {"name": "dashboard_content", "status": "failed", "error": str(e)}
            )
    ...
    def test_performance(self) -> dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/v1/audio/speech",
                json={
                    "input": "This is a test for RTF measurement.",
                    "voice": "af_heart",
                    "response_format": "mp3",
                },
                timeout=30,
            ...
            if response.status_code == 200:
                ...
                if rtf < 1.0:  # Should be faster than real-time
                    results["passed"] += 1
                    results["tests"].append(
                        {"name": "rtf_performance", "status": "passed", "rtf": rtf}
                    )
                else:
                    results["failed"] += 1
                    results["tests"].append(
                        {
                            "name": "rtf_performance",
                            "status": "failed",
                            "error": f"RTF too high: {rtf}",
                        }
                    )
            else:
                results["failed"] += 1
            ...
            results["failed"] += 1
            results["tests"].append(
                {"name": "rtf_performance", "status": "failed", "error": str(e)}
            )
        ...
        try:
            ...
            if response1.status_code == 200 and response2.status_code == 200:
                ...
                if speedup > 2.0:  # Cache should provide significant speedup
                    results["passed"] += 1
                    results["tests"].append(
                        {"name": "cache_performance", "status": "passed", "speedup": speedup}
                    )
                else:
                    results["failed"] += 1
                    results["tests"].append(
                        {
                            ...
                            "status": "failed",
                            "error": f"Low speedup: {speedup}",
                        }
                    )
            else:
                results["failed"] += 1
                results["tests"].append(
                    {"name": "cache_performance", "status": "failed", "error": "HTTP errors"}
                )
        except Exception as e:
            results["failed"] += 1
            results["tests"].append(
                {"name": "cache_performance", "status": "failed", "error": str(e)}
            )
    ...
    def test_error_handling(self) -> dict[str, Any]:
        ...
        error_cases = [
            ...
            (
                {"input": "Test", "voice": "af_heart", "response_format": "invalid"},
                "invalid_format",
            ),
        ...
        for payload, test_name in error_cases:
            ...
            try:
                response = requests.post(
                    f"{self.base_url}/v1/audio/speech", json=payload, timeout=10
                )

                if response.status_code >= 400:  # Should return error
                    results["passed"] += 1
                    results["tests"].append(
                        {"name": test_name, "status": "passed", "status_code": response.status_code}
                    )
                else:
                    results["failed"] += 1
                    results["tests"].append(
                        {
                            "name": test_name,
                            "status": "failed",
                            "error": f"Expected error, got {response.status_code}",
                        }
                    )
            except Exception as e:
                results["failed"] += 1
    ...
    def test_configuration(self) -> dict[str, Any]:
        ...
        if config_path.exists():
            ...
            results["failed"] += 1
            results["tests"].append(
                {"name": "config_file_exists", "status": "failed", "error": "config.json not found"}
            )
                    ...
                    results["tests"].append(
                        {
                            ...
                            "status": "failed",
                            "error": f"Port is {config.get('port')}, expected 8080",
                        }
                    )
            except Exception as e:
                results["failed"] += 1
                results["tests"].append(
                    {"name": "port_configuration", "status": "failed", "error": str(e)}
                )
        else:
            results["failed"] += 1
            results["tests"].append(
                {"name": "port_configuration", "status": "failed", "error": "config.json not found"}
            )
