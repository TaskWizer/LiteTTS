...
class PronunciationTestSuite:
    ...
    def test_text_skipping_omission(self) -> bool:
        ...
        test_cases = [
            {"name": "Short text", "input": "Hello world, this is a test.", "min_words": 6},
            {
                "name": "Medium text",
                "input": " ".join(["word"] * 50) + ".",
            ...
            {"name": "Unicode text", "input": "Hello 世界 🌍 café naïve résumé", "min_words": 5},
        ]
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            if output_words >= test_case["min_words"]:
                ...
                self.log(
                    f"FAIL: Significant text loss (expected >= {test_case['min_words']})", "FAIL"
                )
                all_passed = False
                passed = False
    ...
    def test_api_pronunciation(self, base_url: str = "http://localhost:8354") -> bool:
        ...
        for i, test_text in enumerate(test_cases, 1):
            ...
            try:
                ...
                response = requests.post(f"{base_url}/v1/audio/speech", json=payload, timeout=60)
                end_time = time.time()
