...
def test_openwebui_null_format():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if audio_size > 1000:
                log_test(
                    "OpenWebUI: Null Format",
                    True,
                    f"Generated {audio_size} bytes, defaulted to mp3",
                )
                return True
            else:
...
def test_openwebui_missing_format():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test(
                "OpenWebUI: Missing Format", False, f"HTTP {response.status_code}: {error_data}"
            )
            return False
...
def test_openwebui_null_speed():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if audio_size > 1000:
                log_test(
                    "OpenWebUI: Null Speed", True, f"Generated {audio_size} bytes, defaulted speed"
                )
                return True
            else:
        ...
        if response.status_code == 200:
            ...
            if audio_size > 1000:
                log_test(
                    "OpenWebUI: All Nulls",
                    True,
                    f"Generated {audio_size} bytes, all defaults applied",
                )
                return True
            else:
...
def test_openwebui_valid_request():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test(
                "OpenWebUI: Valid Request", False, f"HTTP {response.status_code}: {error_data}"
            )
            return False
...
def test_openwebui_edge_cases():
    ...
    edge_cases = [
        {
            "name": "Number Format",
            "payload": {"input": "Test", "voice": "af_heart", "response_format": 123},
        },
        {
            "name": "Boolean Format",
            "payload": {"input": "Test", "voice": "af_heart", "response_format": True},
        },
        ...
        try:
            ...
            if response.status_code in [200, 400]:
                ...
                if response.status_code == 200:
                    ...
                    log_test(
                        f"Edge Case: {case['name']}", True, "Failed gracefully with clear error"
                    )
            else:
                log_test(
