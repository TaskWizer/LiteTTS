...
def test_ssml_background():
    ...
    for i, test_case in enumerate(test_cases):
        ...
        # Make TTS request
        payload = {"input": test_case["ssml"], "voice": "af_heart", "response_format": "mp3"}

        try:
            start_time = time.time()
            response = requests.post(
                "http://localhost:8354/v1/audio/speech", json=payload, timeout=30
            )
    ...
    background_tests = [
        ...
        if "background" in r["test"].lower() and r["test"] != "No Background (Control)"
    ]
    ...
    if background_tests and control_tests:
        ...
        if (
            avg_bg_duration > avg_control_duration * 0.8
        ):  # Background tests should be similar duration
