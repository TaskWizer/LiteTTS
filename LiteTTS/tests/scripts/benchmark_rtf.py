...
def benchmark_single_synthesis(
    text: str, voice: str = "af_heart", iterations: int = 5
) -> dict[str, float]:
    ...
    try:
        ...
        for i in range(iterations):
            ...
            logger.info(
                f"Iteration {i + 1}: RTF={rtf:.3f}, synthesis={synthesis_time:.3f}s, audio={audio_duration:.3f}s"
            )
...
def benchmark_batch_synthesis(texts: list[str], voice: str = "af_heart") -> dict[str, float]:
    ...
    try:
        ...
        results = {
            ...
            "single_threaded_rtf": single_time / total_audio_duration
            if total_audio_duration > 0
            else float("inf"),
            "batch_time": batch_time,
            "batch_rtf": batch_time / total_audio_duration
            if total_audio_duration > 0
            else float("inf"),
...
def run_comprehensive_benchmark() -> dict[str, Any]:
    ...
    for i, text in enumerate(test_texts):
        text_label = (
            f"text_{i + 1}_{'short' if len(text) < 50 else 'medium' if len(text) < 150 else 'long'}"
        )
    ...
    if rtf_values:
        results["summary"] = {
            ...
            if statistics.mean(rtf_values) < 0.3
            else "good"
            if statistics.mean(rtf_values) < 0.5
            else "acceptable"
            if statistics.mean(rtf_values) < 1.0
            else "poor",
        }
...
def main():
    ...
    parser.add_argument(
        "--voice", "-v", type=str, default="af_heart", help="Voice to use for testing"
    )
    parser.add_argument(
    ...
    try:
        if args.text:
            ...
            logger.info(
                f"Benchmarking custom text: '{args.text[:50]}{'...' if len(args.text) > 50 else ''}'"
            )
