...
class WhisperImplementationTester:
    ...
    def test_faster_whisper(
        self, model_size: str = "base", compute_type: str = "int8"
    ) -> TestResult:
        ...
        try:
            ...
            segments, info = model.transcribe(
                self.test_audio_path, beam_size=5, language="en", condition_on_previous_text=False
            )
    ...
    def run_all_tests(self, audio_duration: float = 10.0) -> list[TestResult]:
        ...
        for test_type, config in test_configs:
            try:
                ...
                if result.success:
                    logger.info(
                        f"✅ {result.model_name}: RTF={result.rtf:.3f}, Memory={result.peak_memory_mb:.1f}MB"
                    )
                else:
    ...
    def _generate_recommendations(self, results: list[TestResult]) -> list[str]:
        ...
        if rtf_target_met:
            ...
            recommendations.append(
                f"Primary recommendation: {fastest.model_name} (RTF: {fastest.rtf:.3f})"
            )

            lowest_memory = min(rtf_target_met, key=lambda x: x.peak_memory_mb)
            if lowest_memory != fastest:
                recommendations.append(
                    f"For memory-constrained devices: {lowest_memory.model_name} ({lowest_memory.peak_memory_mb:.1f}MB)"
                )
        else:
