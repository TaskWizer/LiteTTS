...
def test_performance_targets():
    ...
    for test_case in test_cases:
        try:
            ...
            response = requests.post(
                f"{BASE_URL}/v1/audio/speech",
                json={"input": test_case["text"], "voice": "af_heart"},
                headers={"Content-Type": "application/json"},
                timeout=60,
            )
            ...
            if response.status_code == 200:
                ...
                if rtf <= test_case["target_rtf"]:
                    logger.info(
                        f"   ✅ {test_case['name']}: RTF={rtf:.3f} (target: {test_case['target_rtf']})"
                    )
                else:
                    logger.warning(
                        f"   ⚠️ {test_case['name']}: RTF={rtf:.3f} exceeds target {test_case['target_rtf']}"
                    )
                    all_passed = False
            else:
    ...
    for scenario in test_scenarios:
        try:
            ...
            if response.status_code == 200 and len(response.content) > 1000:
                ...
                logger.error(
                    f"   ❌ {scenario['name']}: FAILED ({response.status_code}, {len(response.content)} bytes)"
                )
                all_passed = False
    ...
    try:
        ...
        for voice in test_voices:
            try:
                response = requests.post(
                    f"{BASE_URL}/v1/audio/speech",
                    json={"input": f"Testing voice {voice}", "voice": voice},
                    headers={"Content-Type": "application/json"},
                    timeout=30,
                )
    ...
    for test_case in error_test_cases:
        try:
            ...
            if response.status_code == test_case["expected_status"]:
                ...
                logger.warning(
                    f"   ⚠️ {test_case['name']}: Expected {test_case['expected_status']}, got {response.status_code}"
                )
...
def test_concurrent_requests():
    ...
    def make_request(request_id):
        try:
            ...
            response = requests.post(
                f"{BASE_URL}/v1/audio/speech",
                json={"input": f"Concurrent request {request_id}", "voice": "af_heart"},
                headers={"Content-Type": "application/json"},
                timeout=60,
            )
            ...
            results_queue.put({"id": request_id, "error": str(e), "success": False})
