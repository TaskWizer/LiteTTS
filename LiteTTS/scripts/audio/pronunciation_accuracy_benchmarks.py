...
class PronunciationAccuracyBenchmarker:
    ...
    def calculate_pronunciation_accuracy_heuristic(
        self, expected: str, input_text: str, test_case: dict[str, Any]
    ...
    async def benchmark_single_case(
        self, test_case: dict[str, Any], voice: str = "af_heart"
    ) -> PronunciationBenchmark:
        ...
        pronunciation_accuracy, wer, phoneme_accuracy = (
            self.calculate_pronunciation_accuracy_heuristic(
                test_case["expected_pronunciation"], test_case["input_text"], test_case
            )
        )
        ...
        logger.info(
            f"Completed: {test_case['test_id']} - Accuracy: {pronunciation_accuracy:.3f}, WER: {wer:.3f}, RTF: {rtf:.3f}"
        )
        return benchmark
    ...
    async def run_comprehensive_benchmarks(self, voice_models: list[str] = None) -> dict[str, Any]:
            ...
            if voice_benchmarks:
                voice_performance[voice] = {
                    "avg_accuracy": statistics.mean(
                        b.pronunciation_accuracy for b in voice_benchmarks
                    ),
                    "avg_wer": statistics.mean(b.word_error_rate for b in voice_benchmarks),
                    "avg_rtf": statistics.mean(
                        b.rtf for b in voice_benchmarks if b.rtf != float("inf")
                    ),
        ...
        for category in categories:
            ...
            if category_benchmarks:
                category_performance[category] = {
                    "avg_accuracy": statistics.mean(
                        b.pronunciation_accuracy for b in category_benchmarks
                    ),
                    "avg_wer": statistics.mean(b.word_error_rate for b in category_benchmarks),
                    "avg_rtf": statistics.mean(
                        b.rtf for b in category_benchmarks if b.rtf != float("inf")
                    ),
                    "total_tests": len(category_benchmarks),
                    "critical_failures": sum(
                        1 for b in category_benchmarks if b.pronunciation_accuracy < 0.8
                    ),
                }

        # Establish baseline metrics
        baseline_metrics = self._establish_baseline_metrics(
            all_benchmarks, category_performance, voice_performance
        )
        ...
        report = {
            ...
            "recommendations": self._generate_benchmark_recommendations(
                baseline_metrics, category_performance
            ),
        }
    ...
    def _establish_baseline_metrics(
        self,
        benchmarks: list[PronunciationBenchmark],
        ...
        for cat, perf in category_performance.items():
            ...
            if perf["critical_failures"] > 0:
                critical_issues.append(
                    f"Critical failures in {cat}: {perf['critical_failures']} tests"
                )
    ...
    def _generate_performance_summary(
        self, benchmarks: list[PronunciationBenchmark]
    ) -> dict[str, Any]:
    ...
    def _generate_benchmark_recommendations(
        self, baseline: BaselineMetrics, category_performance: dict[str, Any]
    ) -> list[str]:
        ...
        if baseline.overall_accuracy < 0.9:
            recommendations.append(
                f"Improve overall pronunciation accuracy from {baseline.overall_accuracy:.3f} to target 0.9+"
            )
        ...
        for category, accuracy in baseline.category_accuracies.items():
            if accuracy < 0.85:
                recommendations.append(
                    f"Focus on {category} category: accuracy {accuracy:.3f} below target"
                )

        # Performance recommendations
        if baseline.rtf_statistics["mean"] > 0.25:
            recommendations.append(
                f"Optimize processing speed: average RTF {baseline.rtf_statistics['mean']:.3f} exceeds target 0.25"
            )

        # WER recommendations
        if baseline.wer_statistics["mean"] > 0.1:
            recommendations.append(
                f"Reduce word error rate from {baseline.wer_statistics['mean']:.3f} to target <0.1"
            )
        ...
        if worst_voice[1] < 0.8:
            recommendations.append(
                f"Improve {worst_voice[0]} voice model: accuracy {worst_voice[1]:.3f}"
            )
