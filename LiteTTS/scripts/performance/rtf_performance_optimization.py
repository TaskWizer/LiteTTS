...
class RTFPerformanceOptimizer:
    ...
    def _create_rtf_test_cases(self) -> list[RTFTestCase]:
        ...
        ultra_short_texts = ["Hi", "Hello", "Yes", "No", "Thanks", "OK", "Stop", "Go", "Wait"]
    ...
    async def test_single_rtf_case(
        self, test_case: RTFTestCase, voice: str = "af_heart"
    ) -> RTFResult:
        ...
        logger.info(
            f"Completed: {test_case.test_id} - RTF: {rtf:.3f}, Target: {test_case.target_rtf}, Met: {'✅' if target_met else '❌'}"
        )
        return result
    ...
    def calculate_optimization_config(
        self, baseline_results: list[RTFResult]
    ) -> RTFOptimizationConfig:
        ...
        ultra_short_results = [
            r for r in baseline_results if r.test_case.category == "ultra_short" and r.success
        ]
        short_results = [
            r for r in baseline_results if r.test_case.category == "short" and r.success
        ]
        ...
        needs_short_text_optimization = any(
            not r.target_met for r in ultra_short_results + short_results
        )
        ...
        logger.info(
            f"Optimization config: Aggressive: {needs_aggressive_optimization}, Short text focus: {needs_short_text_optimization}"
        )
        return config
    ...
    async def run_optimized_rtf_tests(
        self, config: RTFOptimizationConfig, voice_models: list[str] = None
    ) -> list[RTFResult]:
    ...
    def analyze_rtf_improvements(
        self, baseline_results: list[RTFResult], optimized_results: list[RTFResult]
    ) -> dict[str, Any]:
        ...
        for category in categories:
            baseline_cat = [
                r for r in baseline_results if r.test_case.category == category and r.success
            ]
            optimized_cat = [
                r for r in optimized_results if r.test_case.category == category and r.success
            ]
            ...
            if baseline_cat and optimized_cat:
                ...
                analysis[category] = {
                    ...
                    "rtf_improvement": statistics.mean(baseline_rtf)
                    - statistics.mean(optimized_rtf),
                    "rtf_improvement_percent": (
                        (statistics.mean(baseline_rtf) - statistics.mean(optimized_rtf))
                        / statistics.mean(baseline_rtf)
                    )
                    * 100,
        ...
        if all_baseline and all_optimized:
            ...
            analysis["overall"] = {
                ...
                "rtf_improvement": statistics.mean(baseline_rtf_all)
                - statistics.mean(optimized_rtf_all),
                "rtf_improvement_percent": (
                    (statistics.mean(baseline_rtf_all) - statistics.mean(optimized_rtf_all))
                    / statistics.mean(baseline_rtf_all)
                )
                * 100,
    ...
    def generate_rtf_recommendations(
        self, analysis: dict[str, Any], config: RTFOptimizationConfig
    ) -> list[str]:
        ...
        if overall.get("optimized_avg_rtf", 1.0) > 0.25:
            recommendations.append(
                "Overall RTF still exceeds 0.25 target - consider hardware upgrade or model optimization"
            )
        ...
        if ultra_short.get("optimized_avg_rtf", 1.0) > 0.15:
            recommendations.append(
                "Ultra-short text RTF exceeds 0.15 target - implement specialized short text optimization"
            )

        short = analysis.get("short", {})
        if short.get("optimized_avg_rtf", 1.0) > 0.2:
            recommendations.append(
                "Short text RTF exceeds 0.2 target - focus on text preprocessing optimization"
            )

        # Improvement recommendations
        if overall.get("rtf_improvement_percent", 0) < 10:
            recommendations.append(
                "Limited RTF improvement achieved - consider more aggressive optimization strategies"
            )
    ...
    async def run_comprehensive_rtf_optimization(
        self, voice_models: list[str] = None
    ) -> dict[str, Any]:
        ...
        results = {
            ...
            "summary": self._generate_performance_summary(
                baseline_results, optimized_results, analysis
            ),
        }
    ...
    def _generate_performance_summary(
        self,
        baseline_results: list[RTFResult],
        ...
        summary = {
            ...
            "optimized_success_rate": optimized_success / len(optimized_results)
            if optimized_results
            else 0,
            "baseline_target_compliance": baseline_targets_met / baseline_success
            if baseline_success > 0
            else 0,
            "optimized_target_compliance": optimized_targets_met / optimized_success
            ...
            "critical_targets_met": {
                "ultra_short_rtf_015": analysis.get("ultra_short", {}).get("optimized_avg_rtf", 1.0)
                <= 0.15,
    ...
    try:
        ...
        for category in ["ultra_short", "short", "medium", "overall"]:
            if category in analysis:
                ...
                print(
                    f"  {category.title()}: {cat_data['baseline_avg_rtf']:.3f} → {cat_data['optimized_avg_rtf']:.3f} RTF"
                )
