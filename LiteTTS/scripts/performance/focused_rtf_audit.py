...
class FocusedRTFAuditor:
        ...
        for file_name in key_files:
            ...
            try:
                ...
                sync_patterns = [
                    ...
                    (
                        r"requests\.(?:get|post)\(",
                        "sync_http_in_pipeline",
                        "Synchronous HTTP in pipeline",
                    ),
                    (
                        r"for\s+.*\s+in\s+.*:\s*\n\s*.*\.append\(",
                        "loop_append",
            ...
            try:
                ...
                audio_patterns = [
                    ...
                    (
                        ...
                        "range_len_audio",
                        "Use enumerate or vectorized operations",
                    ),
                    (r"\.astype\(.*\)\.astype\(", "double_conversion", "Double type conversion"),
                    (
                        r"np\.concatenate\(\[.*\]\)",
                        "list_concatenate",
                        "Consider using np.hstack or np.vstack",
                    ),
                ]
    ...
    def generate_focused_recommendations(self):
        ...
        # Analyze findings
        all_issues = self.audit_results.get("performance_issues", []) + self.audit_results.get(
            "rtf_specific_findings", []
        )
    ...
    def save_focused_report(self, output_file: str = "docs/focused_rtf_audit_report.json"):
        ...
        self.audit_results["summary"] = {
            ...
            "optimization_opportunities": len(
                self.audit_results.get("optimization_opportunities", [])
            ),
