        ...
        for file_path in python_files:
            ...
            try:
                ...
                patterns = [
                    (
                        r"\.join\(\[.*for.*in.*\]\)",
                        "string_join_comprehension",
                        "Use generator expression instead of list comprehension with join()",
                    ),
                    (
                        r"len\(.*\)\s*==\s*0",
                        "len_zero_check",
                    ...
                    (
                        r"\.keys\(\)\s*in\s",
                        "dict_keys_in",
                        "Check key directly instead of using .keys()",
                    ),
                    (
                        r"import\s+\*",
                        "wildcard_import",
                        "Avoid wildcard imports for better performance",
                    ),
                ]
                        ...
                        blocking_ops.append(
                            {
                                ...
                                "line": line_num,
                                "severity": "high"
                                if op_type in ["sleep_call", "sync_http"]
                                else "medium",
                                "description": description,
                                "pattern": match.group(),
                            }
    ...
    def analyze_memory_patterns(self):
        ...
        memory_patterns = [
            (
                r"\[\].*for.*in.*range\(.*\)",
                "large_list_creation",
                "Large list creation - consider generators",
            ),
            (r"\.copy\(\)", "object_copy", "Object copying - ensure necessary"),
            (
                r"pickle\.loads?\(",
                "pickle_usage",
                "Pickle usage - consider alternatives for performance",
            ),
            (
                r"json\.loads?\(.*\)",
                "json_parsing",
                "JSON parsing - consider streaming for large data",
            ),
        ]
