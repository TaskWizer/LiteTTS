...
class DockerProductionAuditor:
    ...
    def _check_base_image_security(
        self, lines: list[str], file_path: str
    ) -> list[DockerAuditIssue]:
        ...
        try:
            ...
            if "services" in compose_data:
                for service_name, service_config in compose_data["services"].items():
                    service_issues = self._check_compose_service(
                        service_name, service_config, str(compose_path)
                    )
    ...
    def _check_compose_service(
        self, service_name: str, service_config: dict[str, Any], file_path: str
    ) -> list[DockerAuditIssue]:
    ...
    def run_comprehensive_docker_audit(self) -> DockerAuditResult:
        ...
        production_score = self._calculate_production_readiness_score(
            severity_counts, len(all_issues)
        )
    ...
    def _generate_production_recommendations(
        self, issues: list[DockerAuditIssue], severity_counts: dict[str, int]
    ) -> list[str]:
        ...
        if severity_counts["critical"] > 0:
            recommendations.append(
                f"Address {severity_counts['critical']} critical security issues immediately"
            )

        # High priority issues
        if severity_counts["high"] > 0:
            recommendations.append(
                f"Resolve {severity_counts['high']} high-priority issues before production deployment"
            )
        ...
        for category, category_issues in categories.items():
            if len(category_issues) >= 3:
                recommendations.append(
                    f"Focus on {category} improvements: {len(category_issues)} issues found"
                )
...
def main():
    ...
    try:
        ...
        print("\nCritical Issues:")
        critical_issues = [
            issue for issue in result.detailed_issues if issue.severity == "critical"
        ]
        if critical_issues:
