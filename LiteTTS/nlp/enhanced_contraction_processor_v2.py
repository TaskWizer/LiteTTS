...
class EnhancedContractionProcessorV2:
    ...
    def __init__(self, config: dict | None = None):
        ...
        logger.info(
            "Enhanced Contraction Processor V2 initialized with %d rules",
    ...
    def _load_pronunciation_config(self):
        ...
        try:
            ...
            config_path = os.path.join(
                os.path.dirname(__file__), "..", "config", "enhanced_pronunciation_config.json"
            )
            if os.path.exists(config_path):
                ...
                self.natural_contractions = set(
                    c.lower() for c in contraction_config.get("natural_contractions", [])
                )
                self.problematic_contractions = set(
                    c.lower() for c in contraction_config.get("problematic_contractions", [])
                )
                self.preserve_natural_speech = contraction_config.get(
    ...
    def _init_contraction_rules(self):
        ...
        self.contraction_rules = {
            ...
            "it's": ContractionRule(
                ...
                context_patterns=[
                    (
                        r"it's\s+(been|done|gone|come|become|gotten|taken|given|seen|heard)",
                        "it has",
                    ),
    ...
    def process_contractions(self, text: str, mode: str = "enhanced") -> str:
        ...
        sorted_rules = sorted(
            self.contraction_rules.items(), key=lambda x: x[1].priority.value, reverse=True
        )
            ...
            if rule.context_patterns and f"{contraction_lower}_context" in self.compiled_patterns:
                ...
                for context_pattern, context_expansion in context_patterns:
                    if context_pattern.search(processed_text):
                        ...
                        if main_pattern.search(processed_text):
                            ...
                            if processed_text != old_text:
                                changes_made.append(
                                    f"{contraction} → {context_expansion} (context-aware)"
                                )
                                if self.debug_mode:
                                    logger.debug(
                                        f"Context-aware expansion: {contraction} → {context_expansion}"
                                    )
                            break
                else:
                    ...
                    if main_pattern.search(processed_text):
                        ...
                        if processed_text != old_text:
                            changes_made.append(
                                f"{contraction} → {rule.primary_expansion} (primary)"
                            )
                            if self.debug_mode:
                                logger.debug(
                                    f"Primary expansion: {contraction} → {rule.primary_expansion}"
                                )
            else:
                ...
                if main_pattern.search(processed_text):
                    ...
                    if processed_text != old_text:
                        ...
                        if self.debug_mode:
                            logger.debug(
                                f"Standard expansion: {contraction} → {rule.primary_expansion}"
                            )
    ...
    def validate_processing(self, original_text: str, processed_text: str) -> dict[str, any]:
        ...
        if validation["processing_rate"] < 80:
            validation["issues_found"].append(
                "Low processing rate - many contractions remain unexpanded"
            )
    ...
    def get_statistics(self) -> dict[str, any]:
        ...
        for priority in ContractionPriority:
            priority_counts[priority.name] = sum(
                1 for rule in self.contraction_rules.values() if rule.priority == priority
            )

        context_sensitive_count = sum(
            1 for rule in self.contraction_rules.values() if rule.context_patterns
        )

        phonetic_guidance_count = sum(
            1 for rule in self.contraction_rules.values() if rule.phonetic_hint
        )
