...
class TextNormalizer:
        ...
        try:
            ...
            if config_path.exists():
                with open(config_path) as f:
                    ...
                    logger.debug(
                        f"TextNormalizer config loaded from config.json: expand_contractions={self.expand_contractions}"
                    )
            else:
    ...
    def _compile_number_patterns(self) -> dict[str, re.Pattern]:
        ...
        return {
            ...
            "time": re.compile(
                r"\b([0-1]?[0-9]|2[0-3]):([0-5][0-9])(?::([0-5][0-9]))?\s*(AM|PM|a\.m\.|p\.m\.)?(?=\s|$|[^\w.])",
                re.IGNORECASE,
            ),
        ...
        if RIME_AI_AVAILABLE:
            try:
                ...
                if rime_analysis.confidence_score > 0.7:
                    ...
                    logger.debug(
                        f"RIME AI processing applied with confidence {rime_analysis.confidence_score:.2f}"
                    )
            except Exception as e:
    ...
    def _normalize_abbreviations(self, text: str) -> str:
        ...
        sorted_abbrevs = sorted(
            self.abbreviation_dict.items(), key=lambda x: len(x[0]), reverse=True
        )
    ...
    def _integer_to_words(self, num: int) -> str:
        ...
        if num < 20:
            ...
            return (
                ones[num // 100]
                + " hundred"
                + (" " + self._integer_to_words(num % 100) if num % 100 != 0 else "")
            )
        elif num < 1000000:
            return (
                self._integer_to_words(num // 1000)
                + " thousand"
                + (" " + self._integer_to_words(num % 1000) if num % 1000 != 0 else "")
            )
        elif num < 1000000000:
            return (
                self._integer_to_words(num // 1000000)
                + " million"
                + (" " + self._integer_to_words(num % 1000000) if num % 1000000 != 0 else "")
            )
        else:
    ...
    def _year_to_words(self, year_str: str) -> str:
        ...
        try:
            ...
            if 2000 <= year <= 2009:
                return (
                    f"two thousand {self._integer_to_words(year - 2000)}"
                    if year > 2000
                    else "two thousand"
                )
