...
class HomographResolver:
    ...
    def _compile_context_patterns(self) -> dict[str, list[tuple[re.Pattern, str]]]:
        ...
        patterns["read"] = [
            ...
            (
                re.compile(r"\bread\s+(books?|newspapers?|articles?|stories?)\b", re.IGNORECASE),
                "present",
            ),
            (re.compile(r"\b(yesterday|last|already|had|have)\s+read\b", re.IGNORECASE), "past"),
            (
                re.compile(
                    r"\bread\s+(it|that|this)\s+(yesterday|before|already)\b", re.IGNORECASE
                ),
                "past",
            ),
            (re.compile(r"\bread\s+yesterday\b", re.IGNORECASE), "past"),  # More specific pattern
            (
                re.compile(r"\bI\s+read\s+yesterday\b", re.IGNORECASE),
                "past",
        ...
        patterns["close"] = [
            (
                re.compile(r"\bclose\s+(the|a|an)\s+(door|window|book|eyes?)\b", re.IGNORECASE),
                "shut",
            ),
        ...
        patterns["record"] = [
            (
                re.compile(r"\brecord\s+(a|the)\s+(song|album|video|sound)\b", re.IGNORECASE),
                "capture",
            ),
        ...
        for word in words:
            ...
            if self.pronunciation_dict.has_pronunciation(clean_word):
                ...
                if clean_word != word:
                    ...
                    suffix = (
                        word[word.find(clean_word) + len(clean_word) :]
                        if clean_word in word
                        else ""
                    )
