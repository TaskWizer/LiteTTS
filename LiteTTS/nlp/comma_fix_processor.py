...
class CommaFixProcessor:
    ...
    def _load_problematic_patterns(self) -> list[tuple[str, str, str]]:
        ...
        return [
            ...
            (
                r"(\w+),\s*(if|when|while|because|since|although|though|unless|until)\b",
                r"\1, \2",
                "Comma subordinate clause spacing",
            ),
            (
                r"(\w+),\s*(however|therefore|moreover|furthermore|nevertheless|consequently)\b",
                r"\1, \2",
                "Comma adverbial spacing",
            ),
            (r"(\w+),\s*(which|who|that|where|when)\b", r"\1, \2", "Comma relative clause spacing"),
            (
                r"(\w+),\s*(please|thanks|sorry|excuse me|pardon)\b",
                r"\1, \2",
                "Comma politeness marker spacing",
            ),
        ]
    ...
    def analyze_comma_issues(self, text: str) -> dict[str, list[str]]:
        ...
        for pattern, _, description in self.problematic_patterns:
            ...
            if matches:
                issues["problematic_conjunctions"].extend(
                    [f"{description}: {match}" for match in matches]
                )
