...
class HealthMonitor:
            ...
            try:
                ...
                component_statuses.append(
                    HealthStatus(
                        ...
                        last_check=datetime.now(),
                        response_time_ms=(time.time() - component_start) * 1000
                        if "component_start" in locals()
                        else None,
                    )
                )
    ...
    def _determine_overall_status(
        self, components: list[HealthStatus], system_metrics: dict[str, Any]
    ) -> str:
    ...
    def get_health_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "components_healthy": len(
                [c for c in self.last_health_check.components if c.status == "healthy"]
            ),
            "components_total": len(self.last_health_check.components),
            "memory_usage_mb": self.last_health_check.system_metrics.get("process", {}).get(
                "memory_rss_mb", 0
            ),
            "cpu_usage_percent": self.last_health_check.system_metrics.get("process", {}).get(
                "cpu_percent", 0
            ),
        }
...
def check_model_health():
    ...
    try:
        ...
        return {"status": "unhealthy", "message": f"Model check failed: {e!s}"}
...
def check_disk_space():
    ...
    try:
        ...
        return {"status": "unhealthy", "message": f"Disk check failed: {e!s}"}
