    ...
    def __init__(self, manager: kp.Manager, local_size_x: int = -1, local_size_y: int = -1):
        ...
        if local_size_x < 1:
            if local_size_y > 0:
                local_size_x = 1
                while (
                    2 * local_size_x * local_size_y <= max_workgroup_invocation
                ...
                while 2 * local_size_x * local_size_y <= max_workgroup_invocation:
                    ...
                    if 2 * local_size_y <= max_workgroup_size[1]:
                        ...
                        2 * local_size_x > max_workgroup_size[0]
                    ):  # stop if neither x nor y can be double
                        break
        elif local_size_y < 0:
            local_size_y = 1
            while (
...
{{
    ...
    def __call__(
        ...
        tensor_in_1: kp.Tensor,
        tensor_in_2: kp.Tensor,
        ...
        if self.algo is None or self.tensor_shape != tensor_shape or self.params != params:
            ...
            self.compiled_shader = kp.Shader.compile_source(
                self.shader.format(local_size_x=local_size_x, local_size_y=local_size_y)
            )
...
def main():
    ...
    print(
        f"{experiment_count} matmul time : "
