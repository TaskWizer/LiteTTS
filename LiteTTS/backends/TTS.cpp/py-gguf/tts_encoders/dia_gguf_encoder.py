...
class DiaEncoder(DACEncoder):
    ...
    def __init__(
        self, model_path: Path | str = "./dia.gguf", repo_id: Path | str = DEFAULT_DIA_REPO_ID
    ):
        """
    ...
    def set_gguf_parameters(self):
        ...
        """
        self.set_dac_config(
            self.model.config.data.audio_bos_value, self.model.config.data.audio_eos_value
        )
        # ---- Dia Configuration ----
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.attn_head_size", self.model.config.model.encoder.head_dim
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.eos_token_id", self.model.config.data.audio_eos_value
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.bos_token_id", self.model.config.data.audio_bos_value
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.pad_token_id", self.model.config.data.audio_pad_value
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.max_delay", max(self.model.config.data.delay_pattern)
        )
        ...
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.max_context_length", self.model.config.data.text_length
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.attn_heads", self.model.config.model.encoder.n_head
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.layers", self.model.config.model.encoder.n_layer
        )
        ...
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.hidden_size", self.model.config.model.decoder.n_embd
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.layers", self.model.config.model.decoder.n_layer
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.output_heads", self.model.config.data.channels
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.attn_heads", self.model.config.model.decoder.gqa_query_heads
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.query_heads", self.model.config.model.decoder.kv_heads
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.output_vocab_size", self.model.config.model.tgt_vocab_size
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.audio_vocab_size", self.model.config.data.audio_eos_value
        )
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.max_generation_size", self.model.config.data.audio_length
        )
