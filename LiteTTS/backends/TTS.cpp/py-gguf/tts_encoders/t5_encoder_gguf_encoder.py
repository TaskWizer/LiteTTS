...
class T5Encoder(TTSEncoder):
    ...
    def __init__(
        self,
        model_path: Path | str = "./t5-encoder.gguf",
    ...
    def prepare_tensors(self):
        ...
        self.set_tensor(
            "t5encoder.enc.final_layer_norm",
            self.model.text_encoder.encoder.final_layer_norm.weight,
        )
        for i, layer in enumerate(self.model.text_encoder.encoder.block):
            if i == 0:
                self.set_tensor(
                    f"t5encoder.enc.blk.{i}.attn_rel_b",
                    layer.layer[0].SelfAttention.relative_attention_bias.weight,
                )
            # attention
            ...
            # mlp
            self.set_tensor(
                f"t5encoder.enc.blk.{i}.ffn_up", layer.layer[1].DenseReluDense.wi_0.weight
            )
            self.set_tensor(
                f"t5encoder.enc.blk.{i}.ffn_gate", layer.layer[1].DenseReluDense.wi_1.weight
            )
            self.set_tensor(
                f"t5encoder.enc.blk.{i}.ffn_down", layer.layer[1].DenseReluDense.wo.weight
            )
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_uint32(
            f"{self.gguf_writer.arch}.output_size", self.model.decoder.config.hidden_size
        )
